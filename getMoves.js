...
function getMove(i) {
  axios.get('https://pokeapi.co/api/v2/move/' + i).then(response => {
    ...
    newMove.effectInfo = data.effect_entries[0].effect.replace('$effect_chance', data.effect_chance);
    newMove.shortEffectInfo = data.effect_entries[0].short_effect.replace('$effect_chance', data.effect_chance);
    ...
    // console.log(newMove.shortEffectInfo);
  })
}
...
function addMoveToDatabase(move) {
  db.collection("moves").doc(move.id.toString())
    .set(move)
    .catch(error => {
